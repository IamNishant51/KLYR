...
class Summarizer {
     ...
     */
    summarizeFile(content, filePath) {
     ...
     */
    summarizeFolder(folderPath, fileSummaries, subfolders = []) {
        // Find common purpose
     ...
     */
    summarizeProject(projectRoot, name, directorySummary, packageJsonContent) {
        ...
        const keyFunctions = this.deduplicateStrings(allSummaries.flatMap((s) => s.keyFunctions)).slice(0, 15);
        return {
            projectRoot,
            name,
    ...
    extractExports(content, filePath) {
        ...
        if (filePath.match(/\.(ts|tsx|js|jsx)$/)) {
            const exportMatches = content.match(/export\s+(default\s+)?(class|function|const|interface|type|enum)\s+(\w+)/g);
            if (exportMatches) {
    ...
    extractKeyDefinitions(content, filePath) {
        const defs = [];
    ...
    assessComplexity(lines, functionCount, dependencyCount) {
    ...
    assessFolderComplexity(complexities) {
        const complexCount = complexities.filter((c) => c === 'complex').length;
        const moderateCount = complexities.filter((c) => c === 'moderate').length;
        if (complexCount > complexities.length * 0.5)
            return 'complex';
        if (moderateCount > complexities.length * 0.5 || complexCount > 0)
            return 'moderate';
        return 'simple';
    }
    inferPurpose(filePath, content, exports) {
        // Infer from filename
