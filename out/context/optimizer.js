...
class ContextOptimizer {
    tokenCountEstimation = 4; // ~1 token per 4 characters
    /**
     ...
     */
    createBudget(modelContextWindow = 8000, responseBufferPercent = 0.2) {
        return {
     ...
     */
    optimizeContext(chunks, summaries, conversation, budget) {
        const warnings = [];
        ...
        const contextTokens = this.estimateTokens([...optimizedChunks.map((c) => c.content), ...optimizedSummaries].join('\n'));
        // Step 4: Calculate efficiency
        const totalRelevantContent = chunks.length + summaries.length;
        const efficiency = totalRelevantContent > 0
     ...
     */
    validateContext(context, budget) {
     ...
     */
    selectRelevantChunks(chunks, tokenBudget) {
        if (chunks.length === 0)
            return [];
     ...
     */
    truncateSummaries(summaries, tokenBudget) {
        const result = [];
        let usedTokens = 0;
