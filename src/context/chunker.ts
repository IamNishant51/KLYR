...
export class CodeChunker {
  ...
  private chunkPython(
    ...
    const definitions = [...functionDefs.map((i) => ({ index: i, type: 'function' as const })), ...classDefs.map((i) => ({ index: i, type: 'class' as const }))].sort(
      (a, b) => a.index - b.index
    );
  ...
  private chunkJavaScript(
    ...
    const definitions = [
      ...functionDefs.map((i) => ({ index: i, type: 'function' as const })),
      ...classDefs.map((i) => ({ index: i, type: 'class' as const })),
      ...exportDefs.map((i) => ({ index: i, type: 'block' as const })),
    ]
  ...
  private chunkJava(
    ...
    const definitions = [
      ...classDefs.map((i) => ({ index: i, type: 'class' as const })),
      ...methodDefs.map((i) => ({ index: i, type: 'function' as const })),
    ]
      .filter((v, i, a) => a.findIndex((i2) => i2.index === v.index) === i)
      .sort((a, b) => a.index - b.index);
  ...
  private findPatternIndices(lines: string[], pattern: RegExp): number[] {
