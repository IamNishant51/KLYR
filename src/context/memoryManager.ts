...
export class MemoryManager {
  private shortTerm: MemoryLayer;
  private midTerm: MemoryLayer;
  private longTerm: MemoryLayer;
  ...
  add(entry: MemoryEntry): void {
    ...
    if (layer.entries.length > layer.maxSize) {
      ...
      layer.entries.sort((a, b) =>
        a.importance === b.importance ? b.timestamp - a.timestamp : this.importanceScore(b.importance) - this.importanceScore(a.importance)
      );
  ...
  query(
    ...
    filtered.sort((a, b) => {
      const scoreA = b.timestamp * (b as any).weight;
      const scoreB = a.timestamp * (a as any).weight;
      return scoreA - scoreB;
    });

    return filtered.slice(0, limit).map((e) => {
      const { layer, weight, ...entry } = e as any;
      return entry;
    });
  }
   ...
   */
  getShortTerm(limit: number = 20): MemoryEntry[] {
   ...
   */
  getMidTerm(limit: number = 50): MemoryEntry[] {
   ...
   */
  getLongTerm(type?: string, limit: number = 100): MemoryEntry[] {
  ...
  private cleanExpired(): void {
    ...
    this.shortTerm.entries = this.shortTerm.entries.filter(
      (e) => !e.ttl || e.timestamp + (e.ttl || this.shortTerm.ttl) > now
    );

    this.midTerm.entries = this.midTerm.entries.filter(
      (e) => !e.ttl || e.timestamp + (e.ttl || this.midTerm.ttl) > now
    );
  ...
  getSummary(): {
    ...
    return {
      totalEntries:
        this.shortTerm.entries.length + this.midTerm.entries.length + this.longTerm.entries.length,
      ...
      criticalCount:
        this.shortTerm.entries.filter((e) => e.importance === 'critical').length +
